...
public interface  TaskDetailRepository extends JpaRepository<TaskEntity, UUID> {
    ...
    @Query("select t from TaskEntity t WHERE t.completedDate BETWEEN :fromDate AND :toDate and t.restaurantId = :restaurantId and t.jobStatus = 2 order by t.completedDate")
    ...
    @Query("select t from TaskEntity t WHERE t.completedDate BETWEEN :fromDate AND :toDate and t.restaurantId = :restaurantId and t.jobStatus = 9 order by t.completedDate")
    ...
    @Query("select new DriverPayoutEntity(t.jobId,t.restaurantName,t.customerUsername,t.completedDate,t.tip,t.notes,t.tipInNotesIssue,t.paymentMethod,t.driverPay,t.totalSale,t.fleetId,t.fleetName,t.driverIncome,t.driverCash,t.driverPayout,t.webOrder) from TaskEntity t WHERE t.completedDate BETWEEN :fromDate AND :toDate and t.jobStatus = 2 order by t.fleetName, t.completedDate")
    List<DriverPayoutEntity> getDriverPayout(@Param("fromDate") LocalDateTime fromDate, @Param("toDate") LocalDateTime toDate);

    @Query("select t from TaskEntity t order by t.fleetName, t.completedDate")
    List<TaskEntity> findByOrderByFleetNameAscCreationDateAsc();

    @Query("select distinct t.fleetId from TaskEntity t where t.completedDate BETWEEN :fromDate AND :toDate and t.jobStatus = 2")
    List<Long> findDistinctFleetIdBetweenDates(@Param("fromDate") LocalDateTime fromDate, @Param("toDate") LocalDateTime toDate);

    @Query("select new DriverPayoutEntity(t.jobId,t.restaurantName,t.customerUsername,t.completedDate,t.tip,t.notes,t.tipInNotesIssue,t.paymentMethod,t.driverPay,t.totalSale,t.fleetId,t.fleetName,t.driverIncome,t.driverCash,t.driverPayout,t.webOrder) from TaskEntity t WHERE t.fleetId = :fleetId and t.completedDate BETWEEN :fromDate AND :toDate and t.jobStatus = 2 order by t.completedDate")
    ...
    @Query("select new DriverPayoutEntity(t.jobId,t.restaurantName,t.customerUsername,t.completedDate,t.tip,t.notes,t.tipInNotesIssue,t.paymentMethod,t.driverPay,t.totalSale,t.fleetId,t.fleetName,t.driverIncome,t.driverCash,t.driverPayout,t.webOrder) from TaskEntity t WHERE t.completedDate BETWEEN :fromDate AND :toDate and t.jobStatus = 2 and t.tipInNotesIssue = true order by t.fleetName, t.completedDate")
    ...
    @Query("select new ca.admin.delivermore.collector.data.reportitem.CustomerTasks(t.customerEmail, t.customerUsername, count(t.jobId)) from TaskEntity t where t.createdBy = :createdBy and t.completedDate BETWEEN :fromDate AND :toDate group by nullif(t.customerEmail,'')")
    ...
    @Query("select new ca.admin.delivermore.collector.data.reportitem.CustomerTasks(t.customerEmail, t.customerUsername, count(t.jobId)) from TaskEntity t where t.completedDate BETWEEN :fromDate AND :toDate group by nullif(t.customerEmail,'')")
    List<CustomerTasks> findCustomerTasksByDates(@Param("fromDate") LocalDateTime fromDate, @Param("toDate") LocalDateTime toDate);

    @Query("select count(t.jobId) from TaskEntity t WHERE DATE(t.completedDate) = :date and t.jobStatus = 2")
    Long findTaskCountByDate(@Param("date") Date date);

    @Query("select count(t.jobId) from TaskEntity t where year(t.completedDate) = ?1 and month(t.completedDate) = ?2 and t.jobStatus = 2")
    Long findTaskCountByYearMonth(Integer year, Integer month);

    @Query("select t from TaskEntity t WHERE t.completedDate BETWEEN :fromDate AND :toDate and t.createdBy = 43 and t.globalSubtotal = 0 and t.jobStatus = 2")
    ...
    @Query("select t from TaskEntity t WHERE t.completedDate BETWEEN :fromDate AND :toDate and t.createdBy = 43 and t.paidToVendor IS NULL and t.jobStatus = 2 and t.restaurantId = :restaurantId order by t.completedDate")
    ...
    @Query("select t from TaskEntity t WHERE t.completedDate BETWEEN :fromDate AND :toDate and t.createdBy = 43 and t.jobStatus = 2 and t.restaurantId IN :restaurantIds order by t.completedDate, t.restaurantName")
